...
BEGIN
  ...
  -- Trusted, size-capped bulk import (see import_pins, migration 38) bypasses
  -- the per-actor cap; current_setting(..., true) is NULL for normal inserts.
...
-- Community insights for mods (§9 C1, migration 40). Mod-gated aggregate: weekly
-- pin trend, 30d contributors, subscriber count + 30d growth, top pins, queue backlog.
...
BEGIN
  ...
  result := jsonb_build_object(
    'weekly', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('week', to_char(wk, 'YYYY-MM-DD'), 'count', cnt) ORDER BY wk)
      FROM (
        SELECT gs AS wk,
               (SELECT COUNT(*) FROM pins p
                  WHERE p.community_id = p_community_id AND date_trunc('week', p.created_at) = gs) AS cnt
    ...
    'contributors_30d', (
      SELECT COUNT(DISTINCT user_id) FROM pins
       WHERE community_id = p_community_id AND user_id IS NOT NULL AND created_at > NOW() - INTERVAL '30 days'
    ),
    ...
    'subscribers_30d', (
      SELECT COUNT(*) FROM community_subscriptions
       WHERE community_id = p_community_id AND created_at > NOW() - INTERVAL '30 days'
    ),
    'top_pins', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('id', id, 'title', title, 'vote_count', vote_count) ORDER BY vote_count DESC)
      FROM (
    ...
    'pending_count', (SELECT COUNT(*) FROM pins WHERE community_id = p_community_id AND status = 'pending'),
...
-- Personal map yearbook (§9 C2, migration 41). Self-only aggregate for the signed-in
-- user: pins by month (12), lifetime votes received, communities, photos, routes.
CREATE OR REPLACE FUNCTION public.get_my_yearbook()
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_uid UUID := auth.uid(); result JSONB;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'You must be signed in to view your yearbook'; END IF;
  result := jsonb_build_object(
    'monthly', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('month', to_char(mo, 'YYYY-MM'), 'count', cnt) ORDER BY mo)
      FROM (
        SELECT gs AS mo,
               (SELECT COUNT(*) FROM pins p WHERE p.user_id = v_uid AND date_trunc('month', p.created_at) = gs) AS cnt
        FROM generate_series(date_trunc('month', NOW()) - INTERVAL '11 months', date_trunc('month', NOW()), INTERVAL '1 month') gs
      ) months
    ), '[]'::jsonb),
    'pins_12mo', (SELECT COUNT(*) FROM pins WHERE user_id = v_uid AND created_at > NOW() - INTERVAL '12 months'),
...
-- Bulk-import saved places into a community the caller mods/owns (migration 38).
-- p_places: JSON array of { title, note?, url?, lat, lng }. Caps at 500, validates
-- + clamps each row, skips bad rows, and bypasses the pin rate-limit via the
-- transaction-local mapcrowd.bulk_import flag. Returns the number of pins created.
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Community invite links (§2, migration 44). Mods mint tokens; /join/<token>
-- redeems via redeem_invite(). Only mods can read/manage the rows.
-- ─────────────────────────────────────────────────────────────────────────────
...
CREATE POLICY "invite_links_mod_all" ON community_invite_links FOR ALL
  USING (public.is_community_mod(community_id)) WITH CHECK (public.is_community_mod(community_id));

CREATE OR REPLACE FUNCTION public.redeem_invite(p_token UUID)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_uid UUID := auth.uid(); v_link community_invite_links%ROWTYPE; v_slug TEXT; v_name TEXT; v_private BOOLEAN;
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'This invite link is invalid'; END IF;
  IF v_link.expires_at IS NOT NULL AND v_link.expires_at < NOW() THEN RAISE EXCEPTION 'This invite link has expired'; END IF;
  IF v_link.max_uses IS NOT NULL AND v_link.use_count >= v_link.max_uses THEN RAISE EXCEPTION 'This invite link has reached its use limit'; END IF;
  SELECT slug, name, is_private INTO v_slug, v_name, v_private FROM communities WHERE id = v_link.community_id;
  IF v_private THEN
    INSERT INTO community_members (community_id, user_id, invited_by, status)
  ...
  END IF;
  INSERT INTO community_subscriptions (community_id, user_id) VALUES (v_link.community_id, v_uid) ON CONFLICT DO NOTHING;
...
-- Scavenger-hunt route check-ins (§4 D2, migration 45). Own-rows only; the ~75m
-- distance rule is enforced client-side (a check-in is self-attested).
