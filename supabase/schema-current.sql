...
-- Returns TRUE if the caller is the community owner or an assigned mod.
...
CREATE POLICY "mods_insert_owner" ON community_moderators FOR INSERT
  WITH CHECK (community_id IN (SELECT id FROM communities WHERE created_by = auth.uid()));
...
-- votes (open — session_id is the trust boundary, not auth)
...
-- Vote on a pin (toggle / switch / new); keeps vote_count in sync
...
BEGIN
  ...
  FROM votes WHERE pin_id = p_pin_id AND session_id = p_session_id;

  IF NOT FOUND THEN
    INSERT INTO votes (pin_id, session_id, value) VALUES (p_pin_id, p_session_id, p_value);
  ...
  ELSIF existing_value = p_value THEN
    ...
    UPDATE pins SET vote_count = vote_count - p_value WHERE id = p_pin_id;
  ELSE
    UPDATE votes SET value = p_value WHERE pin_id = p_pin_id AND session_id = p_session_id;
