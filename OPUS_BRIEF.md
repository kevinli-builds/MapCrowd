...
**§1 Notifications + bell SHIPPED (2026-07-27, needs migration 39 live)** — in-app center; rows written ONLY by SECURITY DEFINER triggers (comment on my pin / RSVP to my event / new follower / my pending pin approved|rejected; self-actions + anon-authored pins skipped), no INSERT policy so clients can't forge. `hooks/useNotifications` (own rows w/ actor+pin joins, realtime filtered to user, markAllRead on open) + pure tested `lib/notifications.ts` (per-type wording + tap target) + `NotificationsPanel` (bottom-sheet; rows open the pin or link to the actor's profile). Bell + unread badge in Sidebar header; unread dot on the mobile hamburger (the drawer holding the bell) — **NOTE: put the mobile indicator on the hamburger, not BottomNav as §1 first said, since the hamburger is the entry to the bell.** **Ops: run `supabase/39-notifications.sql`** (adds the table + triggers + realtime publication) or no rows generate. Web push still the deferred later step.
**§9 C1 Mod community-insights ⭐ SHIPPED (2026-07-28, needs migration 40 live)** — an "Insights" tab in `CommunitySettingsModal` (visible to all mods, not just owner/admin) answering "is my community alive?". One mod-gated (`is_community_mod`) SECURITY DEFINER jsonb RPC `get_community_insights` that aggregates across RLS but exposes only counts + public pin titles: pins/week (last 8 weeks, zero-filled), 30d distinct contributors, subscriber count + 30d growth, top-5 live pins by votes, pending-queue count + oldest-wait hours. Pure tested `lib/insights.ts` (`weeklyBars`/`formatPendingLatency`/`growthBadge`); self-contained `components/community/CommunityInsights.tsx` fetches on mount (stat tiles + weekly bar chart + top-pins list). **Ops: run `supabase/40-community-insights.sql`** or the tab errors on load. **§3 Sidebar/PinDetailModal splits SHIPPED (2026-07-28)** — see §3.
**§9 C2 Personal yearbook + C3 Route analytics SHIPPED (2026-07-29)** — **C3** (no migration): RouteBuilder header now shows `mode · N stops · distance · ~time`; pure tested `lib/routeStats.ts` (`routeMeters` prefers snapped `route.geometry`, straight-line spine fallback; `estimateDurationMin` per-mode speeds). Elevation gain still deferred. **C2** (needs migration 41): a "Your year" card on your OWN profile page — a 12-month pin-activity chart + photo/route tiles from the self-only (`auth.uid()`) SECURITY DEFINER `get_my_yearbook` RPC; `components/profile/ProfileYearbook.tsx` (reuses the tested `weeklyBars` scaler), gated on `isOwnProfile`. **Ops: run `supabase/41-profile-yearbook.sql`** or the card fails to load (it fails silently → renders nothing).
**§9 C4 Spring-cleaning + §4 D6 Community Wrapped SHIPPED (2026-07-29, need migrations 42+43)** — both in the mod **Insights** tab. **C4**: `get_stale_pins(community)` (migration 42, mod-gated) = approved live pins >90d old, no net upvotes, no comment in 90d (oldest first, cap 50); the tab lists them with a Remove button (mod delete via `is_pin_owner_or_mod`, optimistic). **D6**: a shareable portrait story card — `get_community_wrapped` (migration 43, mod-gated: totals + this-year pins, contributors, subscriber growth, top pin, most-RSVP'd event, public route count) drawn to a canvas (gradient in the community colour, PNG download) by `components/community/CommunityWrapped.tsx`; pure tested `lib/wrapped.ts` for the card copy; launched from a button atop the Insights tab. **Ops: run `supabase/42-stale-pins.sql` + `supabase/43-community-wrapped.sql`**. **§9 depth roadmap C1–C4 now all shipped.**
**§2 invite links + §4 D1 surprise-me + §4 D2 route check-ins SHIPPED (2026-08-11).** **§2** (migration 44): `community_invite_links` (mod-only RLS) + `redeem_invite` SECURITY DEFINER RPC; `/join/[token]` route (Google sign-in returns to finish → lands on `/c/<slug>`); create/copy/revoke `InviteLinks` manager in the settings Members tab. **D1** (no migration): 🎲 button by Near Me → flies to + opens a random upvoted pin; pure tested `lib/surprise.ts`. **D2** (migration 45): `route_checkins` (own-rows RLS); `RouteHunt` in the route viewer's Stops list — GPS "I'm here" checks in at the nearest stop within ~75m (client-side rule), progress bar + 🏆 finisher. **Ops: run `supabase/44-invite-links.sql` + `supabase/45-route-checkins.sql`** (D1 needs none).
**§9 C3+ route step count + multi-mode times + community breadth SHIPPED (2026-08-19, no migration).** RouteBuilder gained a **trail card** under the header: distance, a per-mode time estimate for ALL four modes (the route's own `travel_mode` highlighted, others read `~` since they share the one length), and a walking **step count** (`estimateSteps`/`formatSteps` in `lib/routeStats.ts`, ~0.75 m stride). **Community breadth**: new pure tested `lib/communityStats.ts` (`computeSpread` → bounding box, E–W/N–S extents, corner-to-corner span, area). Headline is the **span DISTANCE, not bbox area** — area squares outliers and a couple of stray far pins made it read "3.4M km²"; the diagonal-span distance stays legible ("13 km" / "3,935 km") with the span dims + area in the tooltip. Shown as a **"Span" tile** on the public community page — backed by a coords-only (`lat,lng`) sweep of ALL approved live pins (cap 2000), NOT just the 50 shown — and a `· spans N km` line in the in-app `CommunityPinsPanel` (over the pins already in memory). Elevation gain still the only §9 C3 deferral. **Outlier trimming added same day**: `trimOutliers` drops far-flung strays before the box is built (real example "Fun Bars" = 16 Chicago + 15 NYC + 1 LA went 3,935 km → 1,158 km, LA excluded, both cities kept). Uses a **k-nearest-neighbour distance** rule (k=2) + MAD modified-z (3.5), NOT radial-from-centre — the latter fails on bi-city communities because the two real clusters inflate the spread and the stray hides inside it; kNN judges each pin locally. Guards: only trims 6–2000 pins; tooltips note "excludes N far-flung pins" (`spread.trimmed`). **NOT wired to map zoom** — community selection doesn't fit/zoom the map today and the community page has no map; a "zoom to community" fit reusing the trimmed bounds is the natural follow-up if wanted.
**Next → (highest value first)** — §4 D3 time-capsule pins / D4 pin postcards / D5 one-year-ago feed; §6 W4 embeddable community map; §1 web-push (deferred); the `CHECKINS_SPEC.md` feature (below) if you approve its §8 defaults. **§8 mobile SHIPPED (2026-07-13)** — 44px hamburger; route rows py-2.5 on mobile; folder headers/chevrons + all small icon buttons `max-md:p-2` (desktop unchanged); real-device long-press/pinch checks remain the user's. **`CHECKINS_SPEC.md`** is a fully-designed check-ins/outings feature ready to build — it needs the user's four open-decisions (its §8) answered first.
