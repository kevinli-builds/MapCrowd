...
supabase/
  ...
  29-pin-address.sql                       # pins.address (reverse-geocoded label)
  30-public-routes.sql                     # public/community route visibility + RLS
  31-route-geometry.sql                    # routes.travel_mode + cached snapped geometry (ORS)
  32-optional-stops.sql                    # route_pins.step — pins sharing a step are alternatives
  33-route-folders.sql                     # route_folders (sidebar grouping) + routes.folder_id
  34-public-routes-no-community.sql        # public-link routes (is_public, community_id=null)
  35-routes-profiles-fk.sql                # routes.user_id → profiles FK (optional; author embed)
  36-route-branch-geometry.sql             # route_pins.equal_options + routes.branch_geometry (snapped alternatives)
  37-reports.sql                           # abuse reports (pin/comment/photo) → community mod queue + RLS
...
| `reports` | Abuse reports on a pin/comment/photo; `community_id` denormalised on insert (trigger); read/resolve/delete by mods of that community only |
...
SUPABASE_SERVICE_ROLE_KEY=<service role key>   # server-only; used by /api/invite
ORS_API_KEY=<OpenRouteService key>             # server-only; powers /api/route (real route geometry)
```
`ORS_API_KEY` is optional — without it `/api/route` returns 503 and routes draw straight
lines between stops instead of street/trail-following paths. Free key at openrouteservice.org.
...
- **Routes / trails** — ordered pin sequences (`routes` + `route_pins`) drawn as a polyline (`MapInner` `<Polyline>` + auto-fit bounds). Sidebar "Routes" section creates/opens them. Opening a route launches the full-screen **`RouteBuilder`** overlay (modal tier `z-[1200]`, `pointer-events-none` so the single map behind stays the live preview + interactive). Three tabs: **Stops** (numbered list, reorder/remove), **From community** (pick a community → searchable list of its existing pins → Add, the headline way to build a route), **From map** (tap pins to append). Editable name + color (`COMMUNITY_COLORS`). While the builder is open, `activeRouteId` makes pin taps append to the route (`handlePinClick`/`handleMapClick`), and `builderCommunityId` drives which pins the map shows (`mapPins` in `app/page.tsx`). Routes are private by default but an owner can set **visibility** in the builder's ⋯ menu: **Private** / **Public link** (`is_public=true`, `community_id=null`) / **Publish to community** (`is_public=true`, `community_id` set — only offered when ALL stops are in that one community). RLS widens read to public routes + their stops; writes stay owner-only. Community-published routes appear both on the community page (`/c/[slug]`) and in the in-app `CommunityPinsPanel` (Routes section); any route opens via `/?route=<id>` or `handleOpenRouteById` in a read-only viewer (`RouteBuilder canEdit={false}`) — `externalRoute` in `app/page.tsx` holds a non-owned public route. **Real routing**: a route follows streets/trails via OpenRouteService — `routes.travel_mode` (walk/hike/bike/drive) + cached snapped geometry, recomputed (debounced) only by the owner and proxied through `app/api/route/route.ts` (server-only `ORS_API_KEY`); `lib/routing.ts` `fetchRouteGeometry`. Falls back to straight lines if unconfigured/failed. **Optional / branching stops**: pins sharing a `route_pins.step` are alternatives ("…then go to one of these") drawn as light dashed spurs off the previous step's spine — and these spurs are **snapped to streets too** (not crow-flies). The drawing model lives in the pure, tested `lib/route-legs.ts` (`buildRouteLegs` splits a route into **solid runs** = the main path + **dashed legs** = alternative spurs and equal-step main legs; `normalizeSolidSegments` handles legacy flat geometry). The owner's recompute effect snaps each run/leg via ORS and persists them to `routes.geometry` (now an **array of segments** so the solid path can break) + `routes.branch_geometry` (the snapped dashed legs). A per-step **`route_pins.equal_options`** flag (toggle in the Stops list) makes a step's options equal: the incoming main leg is dashed too, so the previous stop fans out to ALL the step's stops as equal dashed branches with no solid default. `MapInner` draws each solid segment with the double-stroke treatment and fits bounds across solid + dashed points.
- **Abuse reporting** — anyone (incl. anonymous) can report a pin or comment via a "Report" flag in `PinDetailModal` (reason picker: spam / inappropriate / wrong location / other). Reports insert into `reports`; a BEFORE INSERT trigger (`set_report_community`) denormalises the target's `community_id` so RLS gates read/resolve/delete to that community's mods (or site admins). Mods work the queue in `CommunitySettingsModal` → **Queue** tab (badge counts pending pins + open reports): each report shows a content preview + reporter, with **Dismiss** (mark resolved) or **Remove** (delete the offending content, then resolve). `target_type` also supports `'photo'` at the schema/trigger level for future use
